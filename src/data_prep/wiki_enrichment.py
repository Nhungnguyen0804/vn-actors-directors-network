...
def extract_summary(wikitext: str) -> str:
    ...
    # Cắt phần trước heading đầu tiên
    parts = re.split(r'(?m)^==[^=].*?==\s*$', wikitext)
    lead = parts[0]

    # Clean toàn bộ
    clean_text = extract_clean_text(lead)
...
#     # XỬ LÝ [[...]] TRƯỚC (để templates không chứa [[...]])
#     # Remove [[A|B]] → B
#     text = re.sub(r'\[\[[^\|\]]+\|([^\]]+)\]\]', r'\1', text)
#     # Remove [[A]] → A
#     text = re.sub(r'\[\[([^\]]+)\]\]', r'\1', text)

#     # Remove templates (SAU khi đã clean [[...]])
#     text = re.sub(r'\{\{.*?\}\}', '', text, flags=re.DOTALL)
...
def extract_clean_text(wikitext):
    ...
    # Xử lý [[...]] với khoảng trắng bảo vệ
    # Remove [[A|B]] → B (có khoảng trắng trước/sau nếu cần)
    text = re.sub(r'\[\[[^\|\]]+\|([^\]]+)\]\]', r' \1 ', text)
    # Remove [[A]] → A (có khoảng trắng trước/sau nếu cần)
    text = re.sub(r'\[\[([^\]]+)\]\]', r' \1 ', text)
    ...
    # Normalize khoảng trắng để xóa khoảng trắng thừa
...
def wiki_xml_to_jsonl(xml_path, output_path, log_every=50_000):
    context = ET.iterparse(xml_path, events=("start", "end"))
    event, root = next(context)
    ...
    print(f"Done. Total pages written: {count}")
...
def run():
    OUTPUT_FILE = "data/wiki_dict.jsonl"
    # load page to wiki_dict.jsonl
    wiki_xml_to_jsonl(XML_FILE, OUTPUT_FILE)
